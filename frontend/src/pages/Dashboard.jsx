  ...
  const fetchDashboardStats = async () => {
    try {
      ...
      if (data.interventions > 0) {
        setFeedEvents(prev => {
          const exists = prev.some(e => e.text.includes("SECURITY BLOCK"));
          ...
          return [
            {
              ...
              sid: "9921",
              time: "Just now",
              text: `SECURITY BLOCK: Sensitive leak or toxicity flagged by AI Supervisor (${data.interventions} total)`,
              pulse: true
            },
            ...prev.slice(0, 3)
          ];
  ...
  return (
    <div className="space-y-8 pb-10">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tight">ECHOMIND Command Center</h1>
          <p className="text-slate-500 mt-1 text-sm font-medium">Real-time monitoring of emotion-aware processors.</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        {/* Chart Area */}
        <GlassCard className="lg:col-span-2 relative overflow-hidden border border-slate-200 hover:border-primary/20 transition-colors duration-500">
          <div className="absolute -right-10 -top-10 w-40 h-40 bg-primary/5 rounded-full blur-3xl pointer-events-none" />
          <div className="absolute -left-10 -bottom-10 w-40 h-40 bg-secondary/5 rounded-full blur-3xl pointer-events-none" />
          <div className="flex items-center justify-between mb-8 relative z-10">
            <div>
              <h2 className="text-xl font-bold text-slate-900 tracking-tight">ECHOMIND Response Analytics</h2>
              <p className="text-xs text-slate-500 font-medium">Emotion satisfaction vs. system stress levels</p>
            </div>
            <select className="bg-white border border-slate-200 rounded-xl text-xs text-slate-800 px-3.5 py-2 focus:outline-none focus:border-primary/50 hover:bg-slate-50 transition-colors cursor-pointer">
        ...
        {/* Live Feed */}
        <GlassCard className="border border-slate-200 hover:border-secondary/20 transition-colors duration-500 flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-xl font-bold text-slate-900 tracking-tight">ECHOMIND AI Supervisor Feed</h2>
                ...
                {feedEvents.map((evt) => (
                  ...
                  >
                    <div className="relative flex-shrink-0">
                      <div className="w-10 h-10 rounded-2xl bg-slate-100 border border-slate-200 flex items-center justify-center group-hover:border-primary/40 group-hover:bg-primary/5 transition-all duration-300">
                        <MessageSquareText className="w-5 h-5 text-slate-400 group-hover:text-primary transition-colors" />
                      </div>
                      {evt.pulse && (
                        <span className="absolute -top-0.5 -right-0.5 flex h-3 w-3">
                          <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75" />
                          <span className="relative inline-flex rounded-full h-3 w-3 bg-emerald-50 border border-white" />
                        </span>
                      )}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center">
                        <h4 className="text-xs font-black text-slate-800 group-hover:text-primary transition-colors">Session #{evt.sid}</h4>
          ...
          <button className="w-full mt-6 py-3 text-[10px] font-black text-primary border border-primary/10 rounded-2xl hover:bg-primary/5 transition-all duration-300 uppercase tracking-widest leading-none active:scale-[0.98]">
            View All Events
          </button>
        </GlassCard>
      </div>
      ...
      <GlassCard className="border border-slate-200 shadow-sm hover:border-primary/30 transition-all duration-500" hover={false}>
        <div className="flex flex-col md:flex-row items-center gap-6">
          <div className="hidden md:flex w-14 h-14 rounded-2xl bg-gradient-premium/20 border border-primary/20 items-center justify-center text-primary shadow-sm flex-shrink-0">
